...
contract SugarLand is ERC20, Ownable {
	using SafeMath for uint256;
	...
	SugarlandDividendTracker public dividendTracker;
	...
	CustomTaxPeriod private _launch2 = CustomTaxPeriod('launch2',0,3600,3,5,5,20,2,15);
	...
	constructor() public ERC20(_name, _symbol) {
		...
		dividendTracker = new SugarlandDividendTracker();
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "Sugarland: The dividend tracker already has that address");
		SugarlandDividendTracker newDividendTracker = SugarlandDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "Sugarland: The new dividend tracker must be owned by the Sugarland token contract");
	...
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "Sugarland: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "Sugarland: Cannot update gasForProcessing to same value");
	...
	function _transfer(
			...
			if(from != owner() && to != owner()) {
				...
				if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
					require(amount <= maxTxAmount, "Sugarland: Buy amount exceeds the maxTxBuyAmount.");
				}
...
contract SugarlandDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("Sugarland_Dividend_Tracker", "Sugarland_Dividend_Tracker") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal override {
		require(false, "Sugarland_Dividend_Tracker: No transfers allowed");
	}
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "Sugarland_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "Sugarland_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "Sugarland_Dividend_Tracker: Cannot update claimWait to same value");
